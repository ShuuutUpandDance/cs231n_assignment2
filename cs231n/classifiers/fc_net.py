...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    ############################################################################
    # hidden_dims: list of the numbers of neurons in each hidden layer
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers - 1):
        ...
        if self.use_batchnorm:
            ...
            if self.use_dropout:    # Forward: drop after batchnorm
    ...
    # Backprop
    dout, dbn, dh, ddrop = {}, {}, {}, {}
    # prop to the last affine layer
    t = self.num_layers - 1
    dout[t], grads['W' + str(t + 1)], grads['b' + str(t + 1)] = affine_backward(dscores, cache)
    # prop to the rest layers
    for i in xrange(t):
        if self.use_batchnorm:
            if self.use_dropout:    # Backward: drop before batch norm
                ddrop[t - 1 - i] = dropout_backward(dout[t - i], cache4[t - 1 - i])
                dout[t - i] = ddrop[t - 1 - i]
            dbn[t-1-i] = relu_backward(dout[t-i], cache3[t-1-i])
            dh[t - 1 - i], grads['gamma' + str(t - i)], \
                grads['beta' + str(t - i)] = batchnorm_backward(dbn[t - 1 - i], cache2[t - 1 - i])
            dout[t - 1 - i], grads['W' + str(t - i)], \
                grads['b' + str(t - i)] = affine_backward(dh[t - 1 - i], cache1[t - 1 - i])
        else:
            if self.use_dropout:
                ddrop[t - 1 - i] = dropout_backward(dout[t - i], cache4[t - 1 - i])
                dout[t - i] = ddrop[t - 1 - i]
            dout[t - 1 - i], grads['W' + str(t - i)], \
                grads['b' + str(t - i)] = affine_relu_backward(dout[t - i], cache3[t - 1 - i])
