...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  """
    ...
    if mode == 'train':
        ...
        # All intermediates needed for the backward pass
        cache = (x_normalized, gamma, beta, sample_mean, sample_var, x, eps)
        # Keep running mean and variance for test-time
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def dropout_forward(x, dropout_param):
  ...
  """
    ...
    if mode == 'train':
        ...
        ###########################################################################
        out = x  # test phase do nothing special because of inverted dropout
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
        ...
        for f in xrange(F):  # fth filter
            for j in xrange(H_new):
                for k in xrange(W_new):
                    res = np.sum(x_padded[i, :, j * s:HH + j * s, k * s:WW + k * s] * w[f]) + b[f]
                    out[i, f, j, k] = res
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
      ...
      for f in xrange(F):  # fth filter
        for j in xrange(H_new):
          for k in xrange(W_new):
            ...
            dx_padded[i, :, j * stride:HH + j * stride, k * stride:WW + k * stride] += \
                w[f] * dout[i,f,j,k]
...
def max_pool_forward_naive(x, pool_param):
  ...
  """
    ...
    N, C, H, W = x.shape
    H_new = 1 + (H - HH) / stride   # without padding, expression is different
    W_new = 1 + (W - WW) / stride
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  """
    ...
    # Move C to its previous dimension
